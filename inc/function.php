...
function addProject(){
	...
	if(isset($_POST['title']) && isset($_POST['content'])){
		...
		$sql = "INSERT INTO `timeline_project`(`name`, `description`, `time`) VALUES ('$title','$content', '$time')";

		if(!@mysql_query($sql ,$conn)){
			return output(OUTPUT_ERROR,"数据库操作失败，请联系管理员");
		}

		$sql = "SELECT * FROM timeline_project WHERE time = '$time' and name = '$title' AND description = '$content' ORDER BY id DESC LIMIT 0 , 1";
